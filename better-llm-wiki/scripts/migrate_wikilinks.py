#!/usr/bin/env python3
"""
migrate_wikilinks.py — Convert legacy `[[wikilinks]]` in a wiki/ tree
into standard MD links [text](wiki/path.md).
...
Handles:
    [[Target]]                 → [Target](wiki/path.md)
    [[Target|Alias]]           → [Alias](wiki/path.md)
    [[folder/Target]]          → [Target](wiki/folder/path.md)
    [[folder/Target#Section]]  → [Target](wiki/folder/path.md#section)
    [[folder/Target/index|T]]  → [T](wiki/folder/Target/index.md)
    [[#Section]]               → [Section](#section)                     (same-file anchor)
    ![[image.png]]             → ![](wiki/assets/image.png)              (best-effort resolve)
...
def build_page_index(
    ...
    """Three indices:
      ...
      - `all_rels`: [(rel_posix, abs_path)]. Used for legacy suffix matching.
    """
...
def lookup_target(
    ...
    # 3. Legacy suffix match: any page whose path ends with `t.md` or `t/index.md`.
...
def wiki_relpath_posix(target_abs: Path, wiki_dir: Path) -> str:
    return "wiki/" + target_abs.relative_to(wiki_dir).as_posix()
...
def rewrite_text(
    ...
    def sub(m: re.Match) -> str:
        ...
        # [[#Section]] — same-file anchor. Empty target means the current page.
