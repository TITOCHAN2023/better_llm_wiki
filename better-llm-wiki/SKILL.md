...
Crash guard: after the audit inbox is clear, lint checks scale before reading any page. It guards on three axes, all measured by `stat` (no page content is read to decide):
- **Page count of the run** — full-graph lint holds per-page structures (preloaded text + node/edge/adjacency/reverse-link maps + sidecar payloads) in memory at once, so peak RSS scales with the number of pages *loaded this run* (~30 KB/page measured), **not** with total bytes. A wiki of many tiny pages OOMs long before any byte limit, so this is the primary guard. Defaults: warn at 25000 pages, hard-stop at 80000. It applies to `--full` and to a cold first incremental run (no stat cache yet) that would otherwise read every page.
- **Total corpus bytes** (full mode only) — warn at 50MB, hard-stop at 1GB.
- **Single-page bytes** — warn at 512KB, hard-stop at 16MB.

If the guard stops, split oversized pages, archive cold material, move evidence-heavy text to `raw/`, and rely on `query_wiki.py`/`.query-index/wiki.db` for discovery — the query index and steady-state incremental lint stay bounded (a one-page edit lints in ~1s regardless of corpus size). Full-graph lint is intentionally bounded; at large scale it is the wrong tool, not a resize target. Override deliberately with `LLM_WIKI_MAX_PAGES_WARN`, `LLM_WIKI_MAX_PAGES_HARD`, `LLM_WIKI_MD_WARN_MB`, `LLM_WIKI_MD_HARD_MB`, `LLM_WIKI_PAGE_WARN_KB`, or `LLM_WIKI_PAGE_HARD_MB`.

Incremental rule (the default): without `--full`, lint detects modified `wiki/**/*.md` files itself. It picks the change-detection strategy automatically:
