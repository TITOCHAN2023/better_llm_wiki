...
Log files are an explicit exception to the wiki page schema: **do not add YAML frontmatter**. The first byte of the file should be the date H1, because `scripts/lint_wiki.py` validates this shape and the recent-activity graph compiler reads operation headings directly.
...
- Retargeted `Example System Architecture` → `tech/example-system/Example_System_Architecture.md` in 2 files
```

Rules:
- One H1 per file, matching the filename date in ISO format (`YYYY-MM-DD`).
- No YAML frontmatter. A log file starting with `---` is invalid.
- One H2 per operation, starting with `## [HH:MM] <op> | <one-line description>`.
- Time is local time, 24h.
- Body is a short bullet list summarising what changed. Reference files touched with standard MD links using content-root paths, e.g. `[name](/concepts/Name.md)`. Do not use `../` in links produced by this skill.

## Ops allowed in the log

| Op | When it appears | Example |
|---|---|---|
| `compile`  | Structural edits, splits, merges, index rebuild | `## [10:00] compile \| split Example System page into 7 sub-pages` |
...
- **Long rationale**: put design decisions and rationale in `SCHEMA.md` "Notes for the LLM", not in the log.
