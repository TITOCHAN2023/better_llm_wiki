...
2. Write `wiki/concepts/<topic>/index.md`:
   ...
   - [<aspect-2>](/concepts/<topic>/<aspect-2>.md) — <one-line summary>
   - ...
...
> All paths inside `wiki/` files use the content-root anchor (leading `/`). **`../` is meaningless — never write it.** From any wiki file, in the default layout: concepts are `/concepts/Other.md`, entities are `/entities/Name.md`, summaries are `/summaries/slug.md`. Wrap paths with spaces in angle brackets: `[Foo](</concepts/Foo with spaces.md>)`.
...
The web viewer renders math server-side with KaTeX. The markdown remains portable to other KaTeX-capable preview tools.

## Link rules

All links inside `wiki/` files use the **content-root anchor** — a leading `/` that resolves at the wiki content root — and end in `.md`. The same form is written everywhere, so **`../` is meaningless — never write it**, and every link omits the old `wiki/` prefix. Wrap any path containing spaces in `<...>` angle brackets.
...
### Content-root paths (same for ALL files under `wiki/`)
| Target | Link |
|---|---|
...
3. Add to the article's "Open questions" section **and** the wiki's `SCHEMA.md` research questions.
