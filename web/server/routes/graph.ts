...
export function buildGraph(wikiRoot: string): GraphData {
  ...
  // Build edges by resolving each MD link to a root-relative `wiki/...` path.
  ...
  for (const f of files) {
    ...
    while ((m = MD_LINK_RE.exec(text))) {
      ...
      const tgtId = resolveGraphTarget(wikiRoot, pathPart);
      if (!tgtId) continue;
...
function resolveGraphTarget(wikiRoot: string, pathPart: string): string | null {
  const normalized = path.posix.normalize(pathPart);
  if (normalized === "." || normalized === ".." || path.posix.isAbsolute(normalized)) {
    return null;
  }

  const candidates = normalized.startsWith("wiki/")
    ? [normalized]
    : [path.posix.normalize(`wiki/${normalized}`)];

  for (const candidate of candidates) {
    if (candidate.startsWith("../")) continue;
    const resolvedAbs = path.resolve(wikiRoot, candidate);
